...
function get_elementor_uploaded_css(){
    ...
    if (strtoupper(substr(PHP_OS, 0, 3)) === 'WIN'){
        $uploads_path = strval(str_replace (substr(str_replace("/","\\",get_home_path()), 0, -1),"",$uploads_dir["basedir"]));
    } else {
        $uploads_path = strval(str_replace (substr(get_home_path(), 0, -1),"",$uploads_dir["basedir"]));
    } 
